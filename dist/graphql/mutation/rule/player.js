...
exports.PlayerMutation = undefined;
...
var input = new _graphql.GraphQLInputObjectType({
  ...
  fields: function fields() {
    return {
      ...
      player_slug: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLString) },
      default_primary: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLString) },
      default_secondary: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLString) },
      id_nba: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLInt) },
      rookie: { type: _graphql.GraphQLBoolean },
      birthdate: { type: _graphql.GraphQLString }
    };
  }
});

var PlayerMutation = exports.PlayerMutation = {
  retirePlayer: {
    ...
    resolve: function resolve(root, _ref) {
      ...
      return _player2.default.update({ retired: true }, { where: { id_player: id_player } });
    }
  },
  savePlayer: {
    ...
    resolve: function resolve(root, _ref2) {
      ...
      return _connection2.default.transaction(function (t) {
        ...
        if (!player.id_player) {
          return _player2.default.create(playerUpdate, { transaction: t }).then(function (createdPlayer) {
            return _setup.Taxonomy.create({ slug: player.player_slug, id: createdPlayer.id_player }, { transaction: t });
          }).then(function (createdTaxonomy) {
            return createdTaxonomy.id;
          });
        }

        var playerQuery = _player2.default.update(playerUpdate, { where: { id_player: player.id_player }, transaction: t });
        var slugUpdate = { slug: player.player_slug };
        var slugQuery = _player2.default.findOne({ where: { id_player: player.id_player } }, { transaction: t }).then(function (p) {
          return _setup.Taxonomy.update(slugUpdate, { where: { slug: p.player_slug }, transaction: t });
        });
