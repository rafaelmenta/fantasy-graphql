...
var TeamMutation = {
  ...
  updateTeamInfo: {
    ...
    resolve: function resolve(root, _ref4) {
      ...
      return _connection["default"].transaction(function (t) {
        return _teamSl["default"].findOne({
          where: {
            id_sl: id_sl
          }
        }).then(function (team) {
          ...
          if (slug) {
            update.slug = slug;
            slugQuery = _setup.Taxonomy.update({
              slug: slug
            }, {
              where: {
                slug: team.slug
          ...
          var updateInfoQuery = _teamSl["default"].update(update, {
            where: {
              id_sl: id_sl
            },
            transaction: t
          ...
          if (id_user) {
            var userTeamQuery = _setup.UserTeam.update({
              id_user: id_user
            }, {
              where: {
                id_sl: id_sl
  ...
  recruitPlayer: {
    ...
    resolve: function resolve(root, _ref5) {
      ...
      return _leagueConfig["default"].findOne({
        where: {
          id_config: 'FREE_AGENCY_LOCKED',
          id_league: id_league
        }
