...
exports.ManualTeamOverviewQuery = void 0;
...
var _teamSeason = _interopRequireDefault(require("../../../model/associations/team-season"));
  ...
  fields: function fields() {
    return {
      contract_salary: {
        ...
        resolve: function resolve(salaries) {
...
var ManualTeamPlayer = new _graphql.GraphQLObjectType({
  ...
  fields: function fields() {
    return {
      ...
      team_info: {
        type: ManualTeamInfo,
        resolve: function resolve(player) {
          return player.team_players[0];
        }
      },
      salary: {
        type: ManualTeamPlayerSalary,
        resolve: _setup.PlayerLeagueSalary.TeamPlayerSalary
      },
      stats: {
        type: new _graphql.GraphQLList(ManualTeamPlayerStat),
        resolve: function resolve(player) {
      ...
      draft: {
        ...
        resolve: function resolve(pick) {
          return _setup.Draft.findOne({
            where: {
              id_draft: pick.id_draft
            }
          });
        }
      },
      original: {
        type: ManualTeamLookup,
        resolve: function resolve(pick) {
...
var ManualTeamOverview = new _graphql.GraphQLObjectType({
  ...
  fields: function fields() {
    return {
      ...
      division: {
        type: ManualTeamDivision
      },
      users: {
        type: new _graphql.GraphQLList(ManualTeamUser),
        resolve: function resolve(team) {
          return team.users;
        }
      },
      players: {
        type: new _graphql.GraphQLList(ManualTeamPlayer),
        resolve: function resolve(team) {
          return team ? team.players : [];
        }
      },
      record: {
        type: ManualTeamRecord,
        resolve: function resolve(team) {
          return team.team_season;
        }
      },
      stats: {
        type: ManualTeamStats,
        resolve: function resolve(team) {
          return team.stats;
        }
      },
      picks: {
        type: new _graphql.GraphQLList(ManualTeamPick),
        resolve: function resolve(team) {
          return team.picks;
        }
      },
      recent_games: {
        type: new _graphql.GraphQLList(ManualTeamGame),
        resolve: function resolve(team) {
...
var ManualTeamOverviewById = function ManualTeamOverviewById(args) {
  ...
  var playersQuery = _setup.Player.findAll({
    include: [{
      model: _setup.TeamPlayer,
      where: {
        id_sl: args.id_sl
      }
    }, {
      model: _setup.PlayerStats,
      include: [{
        model: _setup.Season,
        where: {
          current: true
  ...
  var picksQuery = _setup.Pick.findAll({
    where: {
      id_owner: args.id_sl,
      is_used: false
    }
  ...
  var recentGamesQuery = _setup.TeamSl.RecentGames({
    id_sl: args.id_sl
  }).then(function (games) {
    var performanceConditions = games.map(function (game) {
      return [{
        $or: [{
          id_sl: game.away_team,
          id_round: game.id_round
        }, {
          id_sl: game.home_team,
          id_round: game.id_round
        }]
      }];
    });
    ...
    var performanceQuery = _setup.TeamPerformance.findAll({
      where: {
        $or: performanceConditions
      }
    }).then(function (performances) {
      return performances.forEach(function (performance) {
        teamPerfomanceMap["".concat(performance.id_sl, "_").concat(performance.id_round)] = performance;
      });
    });
    ...
    return Promise.all([games, performanceQuery]).then(function (results) {
      ...
      games.forEach(function (game) {
        game.home_performance = teamPerfomanceMap["".concat(game.home_team, "_").concat(game.id_round)];
        game.away_performance = teamPerfomanceMap["".concat(game.away_team, "_").concat(game.id_round)];
      });
      return games;
    });
  });
...
exports.ManualTeamOverviewQuery = ManualTeamOverviewQuery;