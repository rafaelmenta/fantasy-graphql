...
exports.ManualTeamRosterQuery = void 0;
...
var ManualTeamRosterPlayer = new _graphql.GraphQLObjectType({
  ...
  fields: function fields() {
    return {
      ...
      team_info: {
        type: ManualTeamRosterTeamInfo,
        resolve: function resolve(player) {
          return player.team_players[0];
        }
      },
      salary: {
        type: ManualRosterPlayerSalary,
        resolve: _playerLeagueSalary.PlayerLeagueSalary.TeamPlayerSalary
      },
      stats: {
        type: ManualTeamRosterPlayerStat,
        resolve: function resolve(player) {
...
var ManualTeamRosterPicks = new _graphql.GraphQLObjectType({
  ...
  fields: function fields() {
    return {
      ...
      draft: {
        type: ManualTeamRosterPickDraft
      },
      original: {
        type: ManualTeamRosterPickTeam
      }
    };
  }
});
var ManualTeamRoster = new _graphql.GraphQLObjectType({
  ...
  fields: function fields() {
    return {
      ...
      players: {
        type: new _graphql.GraphQLList(ManualTeamRosterPlayer)
      },
      picks: {
        type: new _graphql.GraphQLList(ManualTeamRosterPicks)
      }
    };
  }
}); // Export query ------------------------------------------------------------------------------------
...
var TeamRoster = function TeamRoster(root, args) {
  ...
  var picks = _setup.Pick.findAll({
    include: [{
      model: _draft["default"]
    }, _setup.Pick.Original],
    where: {
      id_owner: args.id_sl,
      is_used: false
    }
  ...
  var team = _teamSl["default"].findOne({
    where: {
      id_sl: args.id_sl
    }
  });
...
exports.ManualTeamRosterQuery = ManualTeamRosterQuery;